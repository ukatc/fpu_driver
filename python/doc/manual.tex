...
FPUs with equal firmware versions should have equal checksums, of
...
\chapter{Configuring FPU serial numbers}
...
\texttt{ABORTED} state. FPUs which are in state \texttt{readyForward}
or \texttt{readyReversed}, are put inot state \texttt{RESTING}.  On
...
using the \texttt{writeSerialNumnber()} method. The
