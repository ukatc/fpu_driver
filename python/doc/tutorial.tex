...
\item When changing the movement direction, the absolute number of
  ...
  zero and 120 steps in the opposite direction.
...
\index{gen\_wf()!generating waveforms wih constant acceleration}
...
with the resulting torque linerly increasing over time.
...
\begin{minted}{python}
  fpu_commands.gen_wf(delta_alpha, delta_beta, mode='limacc', unit='degrees',
...
The EtherCAN interface documented here, version 1.3.2, is compatible with firmware
1.5.0 and newer. The following limitations apply:
...
\item[firmware versions < 1.5.0]
  ...
  \item This older firmwares does not support movement of FPUs with
    small stepcount at another point than in the final part of a
    waveform.  It also requires an explicit pause between changes of
    the movement direction. Therefore, they are not compatible to the
    default waveform rulesert 4.  For these firmware versions, the
    ruleset for waveform validity rules needs to be explicityly set to
...
\begin{itemize}
\item For this firmware version, the ruleset for waveform validity
  rules needs to be explicityly set to ruleset 2, in both the
  \texttt{configMotion()} and the \texttt{configPaths()} methods.
...
\item A number of internal classes and types in the C++ driver were
  ...
  discussions. Specifically, the name part ``driver'' was subsituted
