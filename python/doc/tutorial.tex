...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For EtherCAN interface version \input{../../version}}
...
The order of presentation is divided in three parts, as follows:

\begin{enumerate}
\item The first part represents a tutorial aiming at explaining quickly how
to use the software. In section~\ref{sec:settingup}, it is explained
how the EtherCAN interface module is configured. Then, in
...
make sure that the datum switches work before using any datum
operation.
...
\subsection{Installing required dependencies for a new installation}
...
\mint{bash}|git checkout can1|
\mint{bash}|git pull|

\index{can protocol 1!building driver}
\index{protocol versions!building}
\index{can1 branch}
%
\textbf{Note that ``git checkout can1'' is now mandatory. Beginning
  with September 19, 2019, the ``master'' branch of the repository
  does not longer refer to the CAN protocol 1 version, because this
  version is not developer further. To build and use the CAN protocol
  1 driver version (to which this version of the documentation
  corresponds), you have to check out the ``can1'' branch.}

\subsubsection{Using the ESO subversion repository}
...
This EtherCAN interface requires FPU firmware version 1.6.0 to support
...
1.6.x) uses ``communication protocol version 1.6'', which has some
...
versions equal or newer than 1.5.2 and an FPU  firmware that supports the
CAN protocol version 1.6.0 or later. Earlier firmware versions do not
...
\index{EtherCAN interface!initialisation!example}
...
\index{version information!retrieving EtherCAN interface and CAN protocol version!example}
...
that ``v1.5.8'' turns into ``1.5.8''. This version number is
...
\index{version information!checking minimum firmware versions}
...
\index{configMotion()|see{rounding errors}}
The method
\begin{minted}{python}
...
The EtherCAN interface documented here, version 1.5.2, requires
firmware version 1.6.0 or newer. The following limitations apply for
...
\item[firmware versions <= 1.6.0]
  \begin{itemize}
    ...
    \item For these firmware protocol versions, it is not possible to
      ...
      every message -- this is only supported by the upcoming hardware
      protocol 2.
  \end{itemize}
  ...
  \item[firmware versions < 1.6.0]
    \begin{itemize}
    \item Requires to set the driver keyword parameter
      \texttt{can\_command\-priority} to zero, otherwise the firmware
      will not recognise any commands. All CAN priorities will be set
      to zero (which is incorrect). In order to avoid problems with
      unreliable transmission of response messages, it is strongly
      recommended to use newer firmware.
  \end{itemize}
...
\subsection*{Changes in version 1.5.10}
\begin{itemize}
\item The LifeTimeTest.py script was updated to
  track breaking changes in the Python datetime
  library API.
\item Naming of some string constants was corrected.
\end{itemize}


\subsection*{Changes in version 1.5.8}
\begin{itemize}
\item Updated installation instructions. The protocol 1 driver is from
  now referred to as the ``can1'' branch, not any more as the
  ``master'' branch. The ``master'' branch now refers to the tested
  (and hopefully working) protocol 2 code.
\end{itemize}
...
\item Add constants in radian, to assist use of uniform SI units in
  the software.
\item Add long-from installation instructions.
\end{itemize}
...
\begin{itemize}
\item Changes which improve integration with verification software
\item Additional argument in constructor of \texttt{FPUGridDriver}, which allows to
  pass an opened LMDB database.
\item Making several driver methods quieter, to decrease noise in console log
\end{itemize}

\subsection*{Changes in version 1.5.3}
\begin{itemize}
\item Fixed minor bugs in build instructions.
\item Several small internal changes needed for testing verification rig software.
\item Added: Change to allow EtherCAN mock up to simulate obstacle for test of collision detection.
\item Changed: Optionally, made some methods and functions such as \texttt{countedAngles()}
  quieter.
\end{itemize}
...
\subsection*{Changes in version 1.5.0}
\begin{itemize}
...
\subsection*{Changes in version 1.4.1}
\begin{itemize}
...
\subsection*{Changes in version 1.4.0}
\begin{itemize}
...
A more complete list of probably needed packages for a Debian system is
\begin{itemize}
\item gcc-6.3
\item liblmdb0
\item lmdb-utils
...
\item For this functionality, the \texttt{mocpath} python library
  ...
  \texttt{numpy}, \texttt{scipy}, and \texttt{matplotlib}\footnote{At
    ROE, getting the latter two packages requires an installation of
    miniconda2, which means that the above mentioned packages like
    python-lmdb also need to be installed into the miniconda
    environment.}.
\end{itemize}
\end{enumerate}
...
If the installation target set by \texttt{--prefix} was, for example,
\texttt{\$HOME}, the setting needs to be adjusted to
\begin{verbatim}
...
building the module. For the example above, that would be
...
The boost::python library may (rarely) require re-compiling when
far-reaching system upgrades are performed (such as an update of the
system's C library). Otherwise, no further changes to it are needed.
...
"/lib". It can be used in any C++ program. The command ``git checkout
can1'' is needed to build a protocol 1 driver (which this document
refers to). To build a protocol 2 driver, please see the corresponding
updated document.
