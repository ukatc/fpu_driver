...
The library uses a lock file to prevent that different processes write
simultaneously to the same gateway. To access the lock file, all user
accounts which access the driver software need to be member of the
Unix group defined in the environment variable
...
The EtherCAN interface documented here, version 1.4.1, is compatible with firmware
...
\item[firmware versions <= 1.6.0]
  ...
  \item[firmware versions < 1.6.0]
    ...
    \item Requires to set the driver parameter
      ...
      will not recognise any commands. In order to avoid problems with
      reliable transmission of response messages, it is recommended
...
\item Added: Previously, the driver did not check whether an EtherCAN
  ...
  operations (it is not designed for that), and subsequently causes
  the driver to stop operations because they have no consistent
  result. This can in theory also damage FPUs.

  To address that, a check for exclusive access was added, which makes
  sure that no other driver instance on the same computer will access
  the same gateway address at the same time.
