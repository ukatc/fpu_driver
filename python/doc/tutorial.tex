...
\item Fixed an aliasing bug in the processing of counters such as
  ...
  relied upon.  To discern correct data from contaminated data, the
  field value was changed.  Values which use the replacement
  ``counters2'' field name are stored correctly.
  
\item Added: Example script named ``plot\_datum\_aberration.py'' which
  reads the healthlog table of the position database and produces a
  matplotlib plot of datum aberrations and their standard deviations
  as a function of the count of datum operations. Additionally, it can
  produce a plot showing the number of movements over calendar time.
