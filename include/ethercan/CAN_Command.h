...
// Unstuffed message to gateway which holds address, CAN identifier, and
// message
...
// Buffer which holds the unencoded (not byte-stuffed) message to the EtherCAN
// gateway
...
public:
    ...
    CAN_Command(E_CAN_COMMAND _command_code) :
        command_code(_command_code), fpu_id(-1), bcast(false), sequence_number(0) {}
    ...
    // Method which serializes parameters into/ byte array which contains CAN
    // message
    ...
    {
        set_msg_header(can_buffer, buf_len, busid, fpu_canid, bcast,
                       _sequence_number);
    };
    ...
    // Boolean value indicating whether the driver should wait for a response
    ...
    // If this is set, a response will be expected from all FPUs which are not
    // locked.
    ...
    // Function which populates CAN message header, and saves the sequence
    // number in a member variable
                        ...
                        const bool _bcast, const uint8_t _sequence_number)
    {
        ...
        // We use bit 7 to 10 for the command code, and bit 0 to 6 for the FPU
        // bus id.
        ...
        // The CAN identifier is either all zeros (for a broadcast message) or
        // bits 7 - 10 are the proiority and bits 0 - 6 the CAN id of the FPU.
        ...
        // The protocol uses little-endian encoding here (the byte order used
