...
** DONE add configPath() method for absolute positioning with mandatory rounding parameter
** DONE fix bug with summed step counter where numbers are too low
** DONE re-test path generator output with waveform checker script, and driver
** TODO Fix and Update CAN overflow error codes in communication protocol (upper limit -10000 + 2 ** 16 - 1)
