...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 0.4.3 (document version \texttt{\input{../git_version}})}
...
sufficient to determine the cause of the problem. Time-outs for failed
sockets connects are by default very long. For changing the time-out
values of socket connections, see Appendix section ~\ref{sec:socketpars} on
page~\pageref{sec:socketpars}.
...
(This command seems to be need to be run as root, and without the
  \texttt{--user} option, in order to install under
  \texttt{/usr/local}).
...
\section{Changing time-out parameters of socket conenctions}
\label{sec:socketpars}
If a socket connection fails during an ongoing driver operation,
depending on operating system parameters the corresponding error
message can be delayed by a very long time. This is the intentional default
behaviour of socket connections \textendash{} the socket connection by
the normal TCP stack of the operating system kernel and its parameters
are optimized to complete communications even on a highly unreliable
connection.

To shorten the time until an idle socket connection is cut to 30
seconds, the following command can be used from the administrative
user account:
