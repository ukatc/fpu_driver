...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 0.9.0-beta (document version \texttt{\input{../git_version}})
DRAFT - DO NOT USE THIS VERSION FOR ENGINEERING PURPOSES.}
...
additional required knownledge, in section~\ref{sec:protectionrules}
...
probles. Section~\ref{sec:commontasks} explains how to perform common
tasks. Section~\ref{sec:errorhandling} expolains how errors are
...
collisions and limit switch breaches are handled. The final sectuion
...
lists all available commands with a comprehensive list of all their
...
this means the alpha datum position is at -180 degrees, and the beta
...
database. Importantly, it has to be repeated whenever it happens that
the stored position is not matching the actual position. This can only
happen if the FPU is moved by something else than the FPU driver
(or by an earlier version of it).
...
Assuming the initial positions were alpha = -179 degrees and beta = 1
degrees, this should show data similar to the following output:
...
operate FPUs. As usual, the operations are presented that the most
...
operation is tested. Nevertheless, it is highly recommended to test
the switch operation before doing any further operations with the FPU.
...
\begin{itemize}
  ...
  \item Never move FPUs circumventing the driver. If FPUs are moved
    ...
    newly. If any driver instance is controlling such an FPU, before
    correcting the stored positions: exit the driver, and only after
    correcting re-start the driver.
  ...
  \item Never power-cycle an FPU while the driver is connected. If an
    ...
    count which is caused by the firmware reset.
...
operation is (-180, 0) degrees, that both switches are off at
positions (-175, 5.0), and both switches on at positions (-180.2,
-0.2).
