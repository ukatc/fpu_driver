...
\texttt{FPU_DATABASE} needs to be set in the configuration file
...
directory defined in the $FPU_DATABASE environment variable. (It is
also possible to back it up while it is being used.  In this case, the
appropriate live backup tools for LMDB need to be used.)
...
\begin{warning}
  \textbf{WARNING: Before first issuing a \texttt{findDatum()}
    ...
    damage the hardware. The software cannot protect against this.}
\end{warning}
...
This test is not exhaustive, as it does not check for reliability of
the hardware. However, to avoid damage it is highly recommended to
perform it when first putting a new FPU into operation.

To perform this test, it is necessary to know the safe operation
ranges of the alpha and beta arm. These ranges depend on hardware and
FPU firmware and are still somewhat subject to change.  As of July
2018, the currently known limits are described in document [4],
``FPUMovementSafetySpecification'', but may change at a later time.
...
\begin{enumerate}
  ...
  \item Move the FPU so that it is within the normal
    range, not touching the datum switches.
    
    As a normal movement configuration requires
    that a datum search has been performed before,
    it is necessary to pass the \texttt{allow_uninitialized=True}
    flag to the configureMotion command:
...
moved out of its allowed range.
...
\index{limit switch breach} \index{dead zone} \textbf{After a limit
  breach, the alpha arm is in a dead zone in which no hardware
  protection is possible.  Especially for protocol versions <= 1.3, it
  is very important that, when resolving such a limit switch breach,
  the FPU is not moved into the same direction as it was moving when
  triggering the breach.  Otherwise, the FPU would run into the hard
  stop, and there is no further protection which could prevent damage
  in this case}.
