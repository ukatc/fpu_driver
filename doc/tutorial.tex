...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 0.5.4 (document version \texttt{\input{../git_version}})}
...
\index{safety!protecting hardware} Second, the \texttt{findDatum()}
...
current Python script when the method returns.\footnote{
  \index{toubleshooting!failed socket connection}\index{SIGINT}
...
\index{safety!after reset}After a reset, an FPU can
