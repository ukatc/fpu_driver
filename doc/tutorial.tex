...
\item Added: The \texttt{configMotion()} command has now an additional
  version parameter which allows to disable the waveform validity
  check, or use a simpler version of the rule set for the check.  The
  waveform rules defined by the RFE to Software document are the rule
  set version which is enabled by default.
...
\item Added: The driver has additional parameters which allow to
  select a higher upload speed for the upload of waveforms to the
  FPUs, and can optionally disable most waiting for
  confirmations. These options are not enabled by default, because
  they might trigger some non-obvious problems.
