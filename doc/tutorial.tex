...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 0.6.0 (document version \texttt{\input{../git_version}})}
...
\subsection{sec:initialsearch}

\section{Initial datum search}
\label{sec:initialsearch}
\index{datum search!initial}

As mentioned above, the listed example relies on the fact that
the FPUs already have been powered on, and a datum search
was performed before. Based on a valid datum search,
the FPU firmware can decide in which direction the beta
arm can be moved. This mode of operation is called
``automatic'' datum search.

However, when an FPU was just powered on, it lacks this information,
so it cannot perform an automatic search. Instead,
the operator needs to instruct the hardware in which
direction it must move. This is done by passing additional
parameters to the \texttt{findDatum()} command, like this:


\begin{minted}{python}
  gd.findDatum(grid_state, search_modes={0 : SEARCH_CLOCKWISE,
    1: SEARCH_ANTI_CLOCKWISE, 2 : SEARCH_CLOCKWISE}, check_protection=False)
\end{minted}

Here, the integer numbers (0, 1, 2, \ldots) correspond to FPU
numbers, and the symbols \texttt{SEARCH\_CLOCKWISE} and
\texttt{SEARCH\_ANTI\_CLOCKWISE} instruct this FPU in which direction
it should move. Here, ``clockwise'' and ``anti-clockwise'' refer to
the directions when looking from above. FPUs which do not get such an
instruction will not move. An automatic datum search is only possible
after all FPUs have performed such an initial manual
search. Section~\ref{sec:initialsearch} gives more information about
these extra parameters.

\index{safety!datum operation}
\begin{warning}
  \textbf{WARNING: When doing a manual datum search, always
...
both arms of all FPUs. The FPU automatically selects the required
movement direction of the beta arm according to the step counter. This
mode is the normal mode used in instrument operation.


\paragraph{Initial datum search after powering on FPUs}
\label{sec:refinitialsearch}

Because the ``automatic'' datum search relies on the internal step
counters of the FPUs, it cannot be used when an FPU was just powered
on - the FPU firmware lacks the information to perform the datum
search of the beta arm safely. In this case, the search direction
needs to be passed by the operator (or the instrument control
software). The direction needs to be defined for each FPU, like this:
...
\item[\texttt{SKIP\_FPU}] The beta arm is not moved.
...
argument, ``\texttt{check\_protection}, to the value \texttt{False}.
...
moved if the flag \texttt{check\_protection=False} is passed to the
