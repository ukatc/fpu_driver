...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 1.3.0 (document version \texttt{\input{../git_version}}).}
...
with driver version 0.5.3, a very comprehensive log of sent and
...
This driver requires FPU firmware version 1.4.4 to support all regular
...
\index{LMDB database!maintenance, backup}
...
\index{version information!retrieving driver and CAN protocol version!example}
...
\index{grid\_state variable!introduction} Almost all commands to the driver
...
\index{rounding error!in waveforms}
...
any waveforms which could cause undesired behavior in the motors.
...
\item The first non-zero entry in any movement sequence needs to have
  ...
  count of 125, which can be changed as a driver parameter. (To be
  ...
  output from the path planning library -- this is also a driver
...
\item Between consecutive step number values, the absolute value of
  ...
  allowed. (This maximum rate of change is also a driver parameter, see
...
to test preliminary path generation algorithms.
...
\item Parameters are a minimum step count (which is 125, by default),
  ...
  limits are driver parameters, see
...
\item Between consecutive step number values, the absolute value of
  ...
  rate of change is also a driver parameter, see
  section~\ref{sec:driverinitialisation}.
...
This rule set is identical to ruleset version 2, with
two exceptions:
...
\texttt{reverseMotion()} command. At this level, the driver does not
...
\item[unixtime] The UNIX time (number of seconds since January 1,
  1970). This corresponds to the time stamps in the driver log.
...
\index{version information!retrieving driver and CAN protocol version!reference}
...
\item[\texttt{motor\_minimum\_frequency=500}] Is the minimum allowed
  frequency of the stepper motor which the driver will accept when
...
\item[\texttt{waveform\_upload\_pause\_us}] This keyword parameter
  ...
  and for driver version 1.2.0 and newer, it is 50\,000 by default. If
  the value is zero, no waiting is done.

\label{it:confirm_each_step}  
\item[\texttt{confirm\_each\_step}] This keyword parameter is a
  Boolean value which controls whether the driver waits for a
  ...
  1.2.0 driver and subsequent versions, the default value is \texttt{True}.

  Important: As of driver version 1.3.0, this option is not yet fully
  implemented, because the correct implementation requires significant
  structural change to the driver. Correspondingly, the waveform
  upload can fail.
...
\index{load\_paths(()}%
\index{configPath()!loading a path}%
\index{mocpath library}%
\index{dependencies!mocpath module}%
The utility function \texttt{wflib.load\_paths()} is the correct way
to load a path generated by the path generator protoype software so
that the \texttt{configPath()} method described in
...
checks as the driver, and marks each step which does not conform to
...
The driver documented here, version 1.3.0, is partly compatible with
firmware 1.4.4 and newer. The following limitations apply:

\begin{description}
\item[firmware versions <= 1.4.4]
\begin{itemize}
\item For these firmware versions, it is not possible to
  configure a waveform reliably without using a confirmation for every message.
\item The driver allows some waveforms which are not within the safe range
  of the firmware.
  \end{itemize}
\item[firmware versions < 1.4.4]
\begin{itemize}
...
\begin{itemize}
\item Compatibility: By default, this FPU driver version is compatible
  with firmware version 1.4.4 upwards. To use it with earlier firmware
  versions, the driver configuration parameter
  \texttt{firmware\_version\_address\_offset} to the value
  0. Detection of CAN overflow conditions requires at least firmware
  version 1.4.4.

\item Updated: The lifetime test script has been updated
  to match the newest waveform rules.
...
\item Added: The \texttt{configMotion()} command has now an additional
  version parameter, texttt{ruleset\_version} which allows to disable
  the waveform validity check, or use a simpler version of the rule
...
\item Added: In addition to the \texttt{configMotion()} command, the
  ...
  \texttt{loadpaths()} utility function.

\item Changed: The waveform validation check was made less
  strict in some aspects in order to match it better
  with the Software to RFE ICD, and to allow to
  use waveforms as generated by the DNF algorithm.
...
\item Fixed: Handling of cases where the step counter overflows after
  ...
  overflow requires some additional firmware support, which might
  become available with firmware version 1.4.5.)
  
\item Added: A \texttt{load\_waveform()} function was added which
  can load the output from the path generator software
  and use it as a waveform for the \texttt{configMotion()} method.
\item Added: A new driver method named \texttt{configPath()} allows to
  ...
  on the FPUs with minimum quantization error when rounding to step
  counts.
  
\item Added: The driver checks for CAN buffer overflow
  error messages which have been added in firmware version 1.4.4\,.
\item Added: The driver has two additional parameters.  The first,
  ...
  of driver version 1.3.0, the second option is not fully implemented.
  Details are described in~\ref{it:waveform_upload_pause_ms} on
  page~\pageref{it:waveform_upload_pause_ms}.

\item Fixed: Minimum and maximum speed settings from the lifetime test
  script are now passed as instantiation parameters when the driver
  instance is created.
...
\begin{itemize}
\item Compatibility: This driver version is not compatible with later
  ...
  different binary location which can't be found by the driver.
...
\item Added: The driver has now initialisation parameters which
