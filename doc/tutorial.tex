...
\index{rounding errors!avoiding} Because the input to the
...
The function itself does not check the path data for validity - this
happens in the driver when the \texttt{configMotion()} method is
called.
...
The driver documented here, version 1.3.0, is compatible with firmware
...
\item For these firmware protocol versions, it is not possible to
  ...
  message.
\item The driver probably allows some waveforms which are not within
...
\item Added: The driver checks for CAN buffer overflow error messages
...
\item Added: The driver has two additional parameters.  The first,
  ...
  of driver version 1.3.0, the second option is not implemented in the
