...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 1.0.0 (document version \texttt{\input{../git_version}}).}
...
position. This can only happen if the FPU is moved by something else
than the FPU driver. 
...
not running, it can be backed up by copying the content of the
directory defined in the \verb+$FPU_DATABASE+ environment
variable. For this, the appropriate live backup tool for LMDB,
\texttt{mdb\_copy} should to be used. \texttt{mdb\_copy} can also
back-up a database which is in use, for example during long-running
lifetime tests.
...
\subsection{Handling ambiguos positions}
\index{ambiguous positions}
\index{ProtectionError!due to ambiguous position}
When movements are interrupted, due to errors like collisions, or
due to \texttt{abortMotion} messages, it is possible that
positions become ambiguous. It can happen that such an ambiguous position
prevents the software protetcion from allowing a movement.
For example, if the beta arm is only known to be in a range
between -10.0 and +10.0 degrees, the software protection
cannot decide whether it should be moved clock-wise or
anti-clockwise. In this case, an error is thrown.

There are three ways to handle such a distuation:
...
\item The easiest approach is to move the FPU so that its range of
  ...
  any more to move in a unambigous direction. For example, if in the
  case given above, the FPU is moved by 15 degrees anti-clockwise, its
  new position will still be ambiguous, but it will be in the range
  from +5.0 to +25.0 degrees. If the FPU is not in initialized state,
  it will be necessary to pass the keyword parameter
  \texttt{allow\_uninitialized=True} to the corresnponding
...
of the driver, earlier than version 1.0.0. For current driver
...
system is powered off.  A detailed description of all possible
