...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 1.1.1 (document version \texttt{\input{../git_version}}).}
...
between -10.0 and +10.0 degrees, the software protection cannot decide
...
\item The easiest approach is to move the FPU so that its range of
  ...
  from +5.0 to +25.0 degrees. (When the FPU is not in initialized
...
\begin{minted}{python}
  FpuGridDriver.trackedAngles(grid_state, fpuset=[], show_offsets=False, active=False)
\end{minted}
...
is issued, these ranges are updated accordingly.
...
\begin{itemize}
\item Added: The \texttt{configMotion()} command now prints the number
  of configured FPUs if the passed wavetable data did not configure
  all of them. This should avoid confusion if there are old wave
  tables active, or if FPUs are not configured to move.

\item Changed: The \texttt{trackedangles()} method now shows angles
  from the FPU step counters only when the keyword argument
  \texttt{show\_offsets} is set to true. Otherwise, only current
  angles and movement ranges are displayed.
  
\item Fixed: If only a subset of FPUs was configured to move
  directly after a \texttt{resetFPUs()) command, a following
    \texttt{executeMotion()} command would fail,
    without actually starting a movement.
  
\item Fixed: During execution of the \texttt{findDatum()} command, a
  safety tolerance of 0.5 degrees had been added. This was dropped
  since it caused confusion and made error recovery from hardware
  datum time-outs more difficult than needed.
  
\item Fixed: The \texttt{executeMotion()} command could trigger a
  Python KeyError exception when not all FPUs were configured and some
  FPUs had not been not initialised before.
  
\item Fixed: Value of alpha datum angle updated to -180
  degrees. Version numbers in explanations updated.
