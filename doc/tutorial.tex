...
\item Fixed: Changed \texttt{repeatMotion()} and
  \texttt{reverseMotion()} commands to expect amd wait for a response
  from the FPU. For allowing workarounds, this wasn't required in
  earlier versions.  However, a response is now needed for the
  protection wrapper.
