...
database. It has to be repeated whenever it happens that the stored
position is not matching the actual position.
...
should be used to verify that the switch is
working. Section~\ref{sec:datumswitchcheck} on
page~\pageref{sec:datumswitchcheck} explains in detail how this is
done.
...
\begin{itemize}
  ...
  \item Never move FPUs circumventing the driver. If FPUs are moved
    ...
    newly. Before correcting the store positions. If any driver instance
    was controlling such an FPU, exit the driver, correct the position,
    and only after that re-start the driver.
...
\paragraph{Why datum switches need to be tested}

The proper function of the switches is required in order to avoid that
the \texttt{findDatum()} command damages the FPU by performing
out-of-bounds movements and driving the alpha arm of the FPU into the
hard stop.  It also avoids that the beta arm becomes involved in
unneeded collisions which could permanently degrade the precision of
...
\paragraph{Required information} To begin this test, it is necessary
to know the exact safe operation ranges of the alpha and beta
arm. These values depend on hardware and FPU firmware revisions and
are still somewhat subject to change.  As of July 2018, the currently
known limits are described in document [4],
``FPUMovementSafetySpecification.docx'', but are planned to be
...
Such a manual search was required to be performed in earlier versions
of the driver < 1.0.0. For current driver versions, while still
possible, this is not necessary any more since the position database
keeps track of the actual positions.  A detailed description of all
possible parameters for the datum search can be found in
...
\begin{minted}{python}
  FpuGridDriver.findDatum(grid_state, search_modes={},
...
\begin{itemize}
\item Added protection layer which disables most unsafe
  movements unless explicitly enabled by a flag.

\item Passing explicit direction parameters to the initial
  \texttt{findDatum()} command is no longer required, as they can be
  safely derived from positions stored in LMDB.
...
\item The \texttt{soft\_protection} parameter for \texttt{findDatum()}
  ...
  back to checking the step count of an initialised FPU.
...
\item The \texttt{configMotion()} command has an additional parameter
  ...
  where no datum command was completed before.

\item The \texttt{list\_angles()} function has been replaced
  by a more powerful \texttt{countedAngles()} method.

\item It is now possible to view the last configured waveform.
...
\item Fixed bug in code which should have waited for a reverse
  movement to complete, causing the command to return prematurely.
