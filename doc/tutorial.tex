...
\item[\texttt{findDatum()}] in line 20 moves the FPU to the datum
  ...
  additional information to perform a safe datum search, a
  \texttt{DE_PROTECTION\_ERROR} exception is returned, see the
  following section~\ref{sec:initialsearch} for further information).
...
To datum the FPU in this case, the search direction needs to be passed
by the operator (or the instrument control software). The direction
needs to be defined for each FPU, like this:
