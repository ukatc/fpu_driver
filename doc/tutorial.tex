...
If in an FPU, the controller firmeware cannot detect a datum switch
...
\begin{itemize}
\item Added: The FPU database contains counters for the number of
...
\item Added: An additional database tracks the counters for each FPU
  indexed by the count of datum operations. The command
  \begin{minted}{bash}
    fpu-admin healthlog {serialnumber}
  \end{minted}
  allows to read these series.
...
\item Fixed: After an \texttt{resetFPU()} command, a 2.5 sec waiting
  time was inserted. Without that pause, the subsequent
  \texttt{getSerialNumbers()} command fails because the FPU cannot
