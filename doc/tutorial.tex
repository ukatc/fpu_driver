...
the \texttt{check\_protection} flag without a direction specification.
...
\begin{minted}{python}
  gd.findDatum(grid_state, {0: SEARCH_CLOCKWISE}, check_protection=False)
\end{minted}
...
\index{movements!of a sub-set of FPUs}
...
page~\pageref{sec:initializegrid}.
...
\index{datum!alpha arm at negative position}
...
\texttt{check\_protection=False} needs to be passed, which skips the
state check.
...
\begin{minted}{python}
  gd.findDatum(grid_state, check_protection=False)
\end{minted}
