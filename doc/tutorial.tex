...
There are two significant difference which are visible to the users of
the FPU driver module. Both need careful consideration.
...
been reset by a \texttt{resetFPU()} command or by powering off the
FPU.
...
The driver object allows to invoke method calls which control the FPU
hardware. The method names always start with a dot after the name of
the driver object.  So, \texttt{gd.abc()} would call method ``abc'' of
the driver object. Like normal function calls, they usually have
parameters.
...
frequently, a short-hand which many of the example scripts use is the
name \texttt{gs}.
...
can make use of the ability that Python can run any commands which
...
a script have been executed, the interpreter waits for command line
arguments.
...
before starting the datum search, \textbf{the beta arm is in a
  counter-clockwise position relative to the datum switch}. If the
the step counters have valid values, the beta arm needs to be in a
...
\item Except the the last non-zero entry, all step numbers in a
  ...
  minimum value of 125, and at most the maximum value of 500.
...
\item The total number of waveform entries can only be 128\footnote{In
...
to 1005 FPUs. The only requirement is that the Python module is
...
\verb+grid_state.FPU[]+ sequence. So, in order to know what the
state of FPU number 900 is, we simply have to query \verb+grid_state.FPU[900]+.
