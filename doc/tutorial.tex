...
\title{Tutorial for the MOONS FPU grid driver Python library
  module} \subtitle{For driver version 0.9.0 (document version \texttt{\input{../git_version}})}
...
To run administrative Python scripts, it is also recommended to
include the above directory in the PATH environment variable, like
this:
...
a known logical ID, determined by the CAN bus ID explained above.
...
electronics PCB of that FPU is replaced by another one. The serial
number must never be used for another FPU.

The second command stores the position of the FPU in the position
database. It has to be repeated whenever there the stored position is
not correct.
...
This should show data similar to the following output:
...
-180 degrees for the alpha arm, and 0 degrees for the beta arm. One
...
required, represent uncertainty around the arm positions, in which
case the number pairs would represent start and end of an interval of
possible positions.
...
therefore quite important to back it up regularly. When the driver is
not running, it can be simply backed up by copying the content of the
directory defined in the \verb+$FPU_DATABASE+ environment
variable. (It is also possible to back it up while it is being used.
In this case, the appropriate live backup tool for LMDB, \texttt{mdb\_copy} need to be
used.)
...
emergency level. Furthermore, some situations are not covered by it,
...
software protection layer. This can be done using the
``\texttt{soft\_protection}'' keyword argument when calling the
corresponding driver methods. Passing the value
...
0.9.x) uses ``communication protocol version 1'', which has some
...
versions equal or newer than 0.9.0 and an FPU  firmware that supports the
...
page~\pageref{sec:datumswitchcheck} explains how the basic functioning
of the datum switches can be verified.
...
\item[\texttt{findDatum()}] in line 20 moves the FPU to the datum
  ...
  must not have passed the limit switch). If a datum search is not
  possible because the arm positions do not allow that, a
  \texttt{DE\_PROTECTION\_ERROR} exception is returned.
...
\item[\texttt{list\_angles()}] is a function which also displays the
  angles in a \texttt{grid\_state} instance. In difference to the
  \texttt{countedAngles()} method, it does not update the content of
  \texttt{grid\_state}, so it is occasionally useful to display values
...
supported by the firmware. It relies on a few conditions that must be
strictly observed at all times to keep the tracked positions
...
\begin{itemize}
  ...
  \item Never move FPUs circumventing the driver. If FPUs are moved
    outside of the driver, their positions need to be configured
    newly.
  \item Never power-cycle an FPU while the driver is connected. If an
    FPU needs to be re-started, exit the program which uses the
    driver, restart the FPU, and start up the program again. This
    ensures that the driver correctly interprets the changed step
    count which is caused by the reset.
  ...
  \item The protection layer cannot defend FPUs from failures of the
    ...
    before using a \texttt{findDatum()} command.
\end{itemize}
...
\item Resuming protected operations after temporarily disabling the
  protection flag.
\item Recovering from beta collisions.
\item Loss of connectivity on the CAN bus.
\item Loss of the Ethernet connection to the EtherCAN gateways.
\item As of protocol v1, manual recovery of limit switch breaches.
\item Power-cycling an FPU while the driver program is not running.
...
switches of an FPU are working.  The proper function of the switches
is required in order to avoid that the \texttt{findDatum()} command
damages the FPU by performing out-of-bounds movements and driving the
alpha arm of the FPU into the hard stop.  It also avoids that the beta
arm becomes involved in unneeded collisions which could permanently
degrade the precision of positioning operations.

Passing the following test is necessary but not sufficient, as it does
not check for reliability of the hardware or its dynamic
properties. To avoid damage it is highly recommended to perform it
when first putting a new FPU into operation.

To perform this test, it is necessary to know the exact safe operation
ranges of the alpha and beta arm. These values depend on hardware and
FPU firmware revisions and are still somewhat subject to change.  As
of July 2018, the currently known limits are described in document
[4], ``FPUMovementSafetySpecification.docx'', but are planned to be
changed.  The following description assumes that the position after
performing a datum operation is (-180, 0) degrees, that both switches
are off at positions (-175, 5.0), and both switches on at positions
(-180.2, -0.2).

To check whether the datum switches work, perform the following
steps:

\begin{enumerate}
  \item If the driver is connected, disconnect it
  \item Make sure that the FPU database contains the
    correct alpha and beta arm positions for the FPU
    which is examined. This can be done using the
    command
    \begin{minted}{bash}
    ...
    This indicates that the alpha arm is at position -180 degrees,
    using a datum offset of -180 degrees, and the beta arm is at 0
    degrees.
  ...
  \item Connect the driver and run a ping command.
  ...
  \item Now, move the FPU so that it just activates the datum
    switches. This depends on firmware version and offsets used.  In
    addition to the options used in the first movement, it is now
    necessary to disable the protection layer by passing the
    ``\texttt{soft\_protection=False}'' keyword argument.  With
    firmware version 1.3 and an initial position of (-180, 0), this will
    be reached by using the command

    \begin{minted}{python}
      w = gen_wf(-5.2,-5.2)
      gd.configMotion(w, gs, allow_uninitialized=True, soft_protection=False)
      gd.executeMotion(gs)
    \end{minted}
...
calibrated - \texttt{countedAngles()} shows only an approximately
...
probably meaningless, and this is why \texttt{countedAngles()} returns
...
\index{limit switch breach} \index{dead zone} \textbf{Specific for
...
\begin{minted}{python}
  FpuGridDriver.countedAngles(grid\_state, fpuset=[], show_uninitialized=False)
\end{minted}
...
The only differences to the utility function \texttt{list\_angles()}
are that the method always retrieves the current angles, and also
always uses the same alpha datum offset with which the driver was
configured upon initialisation. Therefore, when it is intended to
evaluate angles from a previously retrieved \texttt{grid\_state}
structure \emph{without} modifying it, the function
\texttt{list\_angles()} needs to be used.
...
\begin{minted}{python}
  FpuGridDriver.trackedAngles(grid\_state, fpuset=[])
\end{minted}
...
In addition, the command shows also the interval of absolute angle
...
is issued, this ranges are updated accordingly.

An example output of the command looks like this:

\begin{minted}{python}
  >>> gd.trackedAngles(gs)
  FPU #0: angle = (-180, 0), offsets = (0, 0), stepcount angle= (-180.0, 0.0), \
                  last_wform_range=([],[])
\end{minted}

Here,the current actual angle is at position alpha=-180, beta=0
...
and beta = 20 degrees, the output looks like this:
...
\begin{minted}{python}
  ...
  FPU #0: angle = (-170.009, 19.9965), offsets = (0, 0), \
                     stepcount angle= (-170.008660882, 19.99654097), \
                     last_wform_range=([-180.0, -170.00866088237231],[0.0, 19.996540970036314])
>>>
\end{minted}
...
fpu_driver.E_DriverErrCode.DE_OK
>>> gd.trackedAngles(gs)
FPU #0: angle = (-170.009, 19.9965), offsets = (9.99134, 19.9965), \
...
due to communication failures during a movement, then instead of a
single-value position, the interval of \emph{possible} position values
will be shown.  These intervals are also tracked when uncalibrated
...
\begin{minted}{python}
    FpuGridDriver.findDatum(grid_state, search_modes={}, selected_arm=DASEL_BOTH, fpuset=[],
...
the step counter. The latter behaviour can be disabled by setting the
keyword parameter \texttt{support\_uninitialized\_auto}, which is
...
not clear. In this case, it is perhaps easiest to update the position
database with the true position, as described in
section~\ref{sec:setinitialposition} on
page~\pageref{sec:setinitialposition}. Alternatively, it may also be
feasible to move the FPU so that all possible arm positions are at the
same side of the datum switch.
...
direction is filled in by the driver.
...
The method
\begin{minted}{python}
  FpuGridDriver.configMotion(wavetable, grid_state, fpuset=[], soft_protection=True,
...
\paragraph{Software protection}
\index{software protection} Before accepting a waveform and sending it
...
and the movement is not configured.
...
The software protection can be switched off by passing the keyword
parameter \texttt{soft\_protection=False}. This is normally only
needed when checking the working of the datum switches, as described
in section~\ref{sec:datumswitchcheck} on
...
\subsection{getCurrentWaveTables()}
\index{commands!getCurrentWaveTables()} The method
\texttt{getCurrentWaveTables()} retrieves the currently valid, or the
last configured waveform tables as a dictionary for each FPU.


\subsection{getReversed()}
...
FPU. Using the keyword parameter \texttt{soft\_protection=False}, this
...
The command
\begin{minted}{python}
  FpuGridDriver.freeBetaCollision(fpu_id, direction, fpu_state, soft_protection=True)
\end{minted}
...
\subsection*{Changes in version 0.9.0}
\begin{itemize}
\item Fixed bug in code which waited for a reverse movement
  to complete, causing the command to return prematurely.

\item Added protection wrapper code which disables most unsafe
  movements unless explicitly enabled by a flag.
